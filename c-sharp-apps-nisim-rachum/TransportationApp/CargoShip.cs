...
namespace c_sharp_apps_nisim_rachum.TransportationApp
{
    public class CargoShip : CargoVehicle
    {
        ...
        CargoType cargoType = CargoType.CargoShip;

        public CargoShip(Driver driver, double maxWeight, double maxVolume, Port currentPort, Port nextPort, int distanceToNextPort)
        {
            Driver = driver;
            MaxWeight = maxWeight;
            MaxVolume = maxVolume;
        ...
        public override bool UnLoad(List<IPortable> items)
        {
            ...
            for (int i = 0; i < ItemsToLoad.Count && i < items.Count; i++)
            {
                ...
                {
                    //ItemsToLoad[i].SetLocation(null);
